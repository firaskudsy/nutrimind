...
# DNS-rebinding protection auto-allows localhost but rejects other Host headers
# (e.g. the Docker service name). Set MCP_DNS_REBINDING_PROTECTION=false when the
# server is only reachable on a trusted private network (compose), so the backend
# can connect via http://cronometer-mcp:8001/mcp.
_dns_protect = os.getenv("MCP_DNS_REBINDING_PROTECTION", "true").lower() != "false"
...
def add_food_entry(
    ...
    Args:
        ...
        time: Time the food was eaten as 24-hour "HH:MM" (e.g. "16:30").
              ...
              picks the correct meal slot from that hour (not the current time).
    """
    try:
        group_map = {
            "auto": 0,
...
def log_weight(value: float, unit: str = "lbs", date: str | None = None) -> str:
    """Log the user's body weight to Cronometer as a biometric.

    Args:
        value: Body weight measurement.
        unit: Unit of measurement, "lbs" (default) or "kg".
        date: Date as YYYY-MM-DD (defaults to today).
    """
    try:
        client = _get_client()
...
def get_weight_history(
    ...
    """Get body-weight history from Cronometer over a date range.

    Args:
        unit: "lbs" (default) or "kg".
        start_date: Start as YYYY-MM-DD (defaults to 90 days ago).
        end_date: End as YYYY-MM-DD (defaults to today).
    """
    try:
        client = _get_client()
        data = client.get_biometric_history(
            "weight",
            unit=unit,
            start=_parse_date(start_date),
            end=_parse_date(end_date),
        )
        return _ok({"unit": unit, "history": data})
    except Exception as e:
        return _err(e)
