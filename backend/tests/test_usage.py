...
async def test_record_and_summary(usage_mod):
    usage = usage_mod
    await usage.record_from_response(_fake_response(1000, 200, 0.0012), "anthropic/claude-haiku-4-5", "chat")
    await usage.record_from_response(_fake_response(500, 100, 0.0006), "anthropic/claude-haiku-4-5", "proactive")

    summary = await usage.usage_summary()
    today = summary["today"]
...
async def test_fallback_pricing_when_no_cost(usage_mod):
    ...
    await usage.record_from_response(resp, "anthropic/claude-haiku-4-5", "chat")
    summary = await usage.usage_summary()
